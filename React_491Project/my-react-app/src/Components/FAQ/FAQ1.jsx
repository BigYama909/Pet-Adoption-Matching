...
function FAQ() {
  ...
  const faqItemsLength = 5;
  const accordionRefs = useRef(Array(faqItemsLength).fill(null));

  const toggleFAQItem = (index) => {
    setOpenFAQIndex(openFAQIndex === index ? null : index);
    // Scroll to the accordion item after state update and DOM render
    if (openFAQIndex !== index) {
      setTimeout(() => {
        // Check if the ref at the current index exists before trying to scroll into view
      ...
      }, 50); // Delay to allow time for the content to expand and render
    }
  };
  const faqItems = [
    {
